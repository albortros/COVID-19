...
for TYPE in TypeOfData:
    ...
    YPred_model[iteration]   = Ypredicted
    StdPred_model[iteration] = YPERR
    ...
    plt.plot(x+pred_x, [logistic(i,Par) for i in x+pred_x], 'r',label="Logistic model" )
    plt.legend()
    ...
    plt.ylim((min(Y3)*0.9,max([logistic_derivative(i,ParD) for i in list(X)+pred_x])*1.3))
    plt.grid(linestyle='--',which='both')
